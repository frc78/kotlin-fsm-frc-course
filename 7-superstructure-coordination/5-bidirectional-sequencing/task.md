...
its last *settled* position, even while it's mid-flight. A real robot would
also treat in-flight mechanisms as unsafe — you'd check `commandedTarget` and
`atTarget()`, not just `state`.
