...
The superstructure answers the same question for the whole robot: is it at
its goal state? The robot is at target only if every contributing subsystem
reports that it is. Higher-level FSMs compose this way. A robot is "at" `SCORE_L4` only when
