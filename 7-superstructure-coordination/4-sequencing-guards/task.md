...
| `WaitingForElevator(t)`    | `t.elevator`             | (do not change)            | `elevator.atTarget()`                   |
...
their old commanded values. Both start at `STOWED`/`IDLE`, which matches the
initial settled pose, so they stay there.

When the goal state changes, `periodic()` resets the transition to
`WaitingForElevator(newGoal)` (already provided).
...
2. **`advanceTransition()`**: if the subsystems have settled, advance to the
   next phase.
