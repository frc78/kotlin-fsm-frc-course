package course.l7t1

import frc.stubs.superstructure.Arm
import frc.stubs.superstructure.Elevator
import frc.stubs.superstructure.Intake

enum class RobotState(
    val elevator: Elevator.State,
    val arm: Arm.State,
    val intake: Intake.Mode,
) {
    // The placeholder values below are all (STOWED, STOWED, IDLE). That's
    // correct only for STOWED itself. Update the other three to the values
    // listed in task.md.
    STOWED(Elevator.State.STOWED, Arm.State.STOWED, Intake.Mode.IDLE),
    INTAKE_GROUND(Elevator.State.STOWED, Arm.State.STOWED, Intake.Mode.IDLE),
    SCORE_L4(Elevator.State.STOWED, Arm.State.STOWED, Intake.Mode.IDLE),
    CLIMB_PREP(Elevator.State.STOWED, Arm.State.STOWED, Intake.Mode.IDLE);
}
